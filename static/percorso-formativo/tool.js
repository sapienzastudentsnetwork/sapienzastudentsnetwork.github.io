(() => {
  ...
  class StudyPlanTool {
    constructor(root) {
      this.root = root;
      this.state = {version: VERSION, mode: root.dataset.mode || 'presenza', regulation: 'old', curriculum: 'methodological', selected: [], custom: []};
      this.renderShell();
      this.bind();
      this.restoreLocal();
      this.render();
    ...
    renderShell() {
      this.root.innerHTML = `<div class="sp-toolbar">
        ...
        <label class="sp-field">Ordinamento<select data-field="regulation"><option value="new">Dal 2025/26</option><option value="old">Prima del 2025/26</option></select></label>
    ...
    bind() {
      this.root.addEventListener('change', (event) => {
        ...
        if (field) { this.state[field] = event.target.value; if (field === 'mode' || field === 'regulation') this.state.selected = []; this.persist(); this.render(); }
      });
    ...
    restoreLocal() { try { const saved=JSON.parse(localStorage.getItem(this.storageKey())); if(saved?.version===VERSION)this.state={...this.state,...saved}; } catch (_) { localStorage.removeItem(this.storageKey()); } }
